...
**ラベルの原則**: どの時点で見ても嘘にならない文言を選ぶ。「確認中」「対応中」は出版社が無反応でも活動中に見えるため採らず、「連絡済み」（運営がやった事実のみ主張）とした。**連絡先も書かない**: 正誤表を著者本人が持つ本があり（実例あり）、連絡先は案件ごとに違う。「出版社へ連絡済み」と名乗ると著者に送った案件で嘘になり、「著者・出版社へ」だと両方に送ったように読める。誰に送ったかは投稿ごとの statusNote（運営者からの補足）に書ける。
