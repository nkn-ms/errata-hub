...
しきい値は `src/constants/rate-limits.ts`、実装は `src/lib/rate-limit.ts`。**ユーザー単位の固定窓カウンタを Postgres に置く**。
...
- **固定窓を選んだ**（スライディングウィンドウにしない）。窓の境目で最大 2×limit を許す弱点はあるが、目的は外部コストの青天井を防ぐことで瞬間流量の平滑化ではない。1行1クエリで済む単純さを取った。
...
- **これは invocation 枠を守らない**。拒否する場合も関数は起動する（＝Vercel の Active CPU / Invocations は消費される）。**関数の手前で止められるのは Vercel WAF のレート制限だけ**で、Hobby でもルール1本（IP・JA4 キー／窓は最大10分）使える。IP 単位なので本命の穴（アカウント1個で無制限）は塞げず**代替ではなく補完**。公開前に1本入れる想定。
