...
- **画像の操作は本文と同じ場所・同じボタンで確定する**。PENDING の間は編集画面の中で追加・削除して「更新する」で確定し、連絡後は追記フォームの中で説明と一緒に足して「追記する」で確定する。選んだ瞬間に送る作りは採らない（押していないのに反映済み・キャンセルしても戻らない、では確定ボタンの意味が無くなる）。公開ページの画像は閲覧のみ。
- **追記に添えた画像は追記の行に紐づける**（`ReportImage.addendumId`・null = 投稿本体の画像）。本体の「証拠画像」に混ぜると出版社が見た時点で何があったのかが読めなくなり、本文を凍結して追記を分けた意味が画像の側で消えるため。**枚数の上限は投稿単位**（本体＋追記の合計 = `REPORT_IMAGE_MAX_COUNT`）で、連絡後は本体の画像を消せないので追記に足せる枚数はその残りになる。
