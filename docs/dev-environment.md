...
- Supabase ダッシュボード → Storage → New bucket
  ...
  - サイズを 5MB にする理由: アプリ側の上限は 4MiB（`src/features/report/constants/report-images.ts`）で、ダッシュボードの「MB」が 10進解釈でも**常にアプリ側が先に効く**余裕を持たせるため（バケット側は保険の粗い網）
...
- **2日**なのは、一番長いウィンドウが1日（`RATE_LIMITS.createReport`）だから。
  進行中のウィンドウを消すと制限がその場でリセットされてしまうので、最も長いウィンドウより余裕を持たせる
- 消えても壊れない: このテーブルはアプリの本来のデータではなく、行が無ければ「そのウィンドウの1回目」として
