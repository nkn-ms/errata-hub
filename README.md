...
| **テスト** | 単体 376 件（Vitest）/ e2e 212 件（Playwright） |
...
管理画面から `app/` へ移した7つは、features 内の誰とも繋がらず、呼び出し元も1ページだけだった。
...
| 見た目・操作・アクセシビリティ | e2e（`e2e/`・Playwright）。すべての PR で CI が回し、通らないとマージできない |
