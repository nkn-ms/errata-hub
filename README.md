...
- **認可はサーバー層に集約** — 管理操作の Server Action は実行時に必ず `services/auth.ts` の `requireAdminServerAction` を通し、そこを**唯一の信頼境界**とする（毎回 DB の最新ロールを確認）。レイアウトガードや `proxy.ts`（認証チェックのみ）は、管理 UI の表示制御・未ログインの早期リダイレクトといった補助的な役割で、セキュリティを保証するのは操作ごとの認可判定。
