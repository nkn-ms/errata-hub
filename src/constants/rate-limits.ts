// レート制限のしきい値。実装は src/lib/rate-limit.ts。
//
// なぜ要るか: 危ない入口はすべてログイン必須だが、認証が確認するのは「誰か」だけで
// 「何回まで」は見ていない。アカウントは無料で作れるので、1個作られた時点で守りが無くなる。
// 守りたいのは主に外部コストで、①Supabase Storage の容量（画像）②Google Books API の
// 無料枠（プロジェクト全体で共有＝1人が使い切ると全員が検索できなくなる）の2つ。
//
// 悪意が無くても効く: デバウンス（book-search.tsx）が改修で壊れる・リトライが暴走する等、
// 自分のバグに対する保険でもある。
//
// 値の考え方: 「まっとうな利用がぶつからない」ことを最優先にした緩めの設定。
// 絞り込みは実トラフィックを見てから行う（公開前の今は実測値が無いので当て推量になる）。
export const RATE_LIMITS = {
  // 投稿。熱心な投稿者でも1日10件は超えない見込み。
  // 本体に付く画像の総量もここが天井になる（1投稿5枚 × 20 = 1日100枚 = 最大400MB）
  createReport: { limit: 20, windowSec: 24 * 60 * 60 },

  // 追記。⚠️ **画像の枠を追記側にも与えたので、追記の作成回数が総量に効く**。
  // 追記1件あたりではなく投稿あたりで枠を切ってあるため青天井にはならないが、
  // 「投稿を増やさずに追記だけ量産する」経路を塞ぐのはここ。
  // まっとうな使い方（気づくたびに足す）は1日数件で、ぶつからない値にしてある。
  addReportAddendum: { limit: 30, windowSec: 60 * 60 },

  // 画像アップロード。上の投稿上限が日の天井を決めるので、これはバーストを均すだけの役割
  reportImageUpload: { limit: 30, windowSec: 60 * 60 },

  // 書籍検索（Google Books）。分のウィンドウは 400ms デバウンスのタイプアヘッド用で、
  // 打ちながら検索しても通常はぶつからない。日のウィンドウが無料枠（1,000/日・全体共有）の防波堤。
  // ⚠️ guardsExternalQuota: 開発環境でも効かせる。dev から使った分も同じ無料枠を減らすので、
  //    ここを外すと「開発中に本番の検索が止まる」ことが起こりうる（実装は lib/rate-limit.ts）
  booksSearchPerMinute: { limit: 30, windowSec: 60, guardsExternalQuota: true },
  booksSearchPerDay: { limit: 300, windowSec: 24 * 60 * 60, guardsExternalQuota: true },

  // OpenBD 照会。1回の検索で書籍検索より多く飛ぶ（結果の書誌補正で最大20件をまとめて引く）ため緩め。
  // 無料枠は公表されていないが、相手は実在の公共 API なので dev からの分も本物のアクセスになる
  booksOpenbd: { limit: 60, windowSec: 60, guardsExternalQuota: true },

  // 賛同。1人1票は @@unique で担保済みなのでリスクは低いが、連打での書き込みは抑える
  toggleUpvote: { limit: 60, windowSec: 60 },
} as const;
