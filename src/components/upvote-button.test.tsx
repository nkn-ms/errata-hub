...
describe("UpvoteButton", () => {
  it("未ログイン（guest）はクリックで /login へ誘導し API を呼ばない", () => {
    render(<UpvoteButton reportId="r1" initialCount={0} initialUpvoted={false} viewer="guest" />);
  ...
  it("投稿者本人（owner）はボタンが無効", () => {
    render(<UpvoteButton reportId="r1" initialCount={2} initialUpvoted={false} viewer="owner" />);
  ...
  it("未賛同の user がクリックすると POST され、レスポンスの count に更新される", async () => {
    ...
    render(<UpvoteButton reportId="r1" initialCount={2} initialUpvoted={false} viewer="user" />);
    fireEvent.click(screen.getByRole("button"));
  ...
  it("賛同済みの user がクリックすると DELETE され、取り消しの count に更新される", async () => {
    ...
    render(<UpvoteButton reportId="r1" initialCount={2} initialUpvoted={true} viewer="user" />);
    fireEvent.click(screen.getByRole("button"));
