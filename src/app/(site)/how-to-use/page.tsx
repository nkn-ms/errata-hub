...
const STEPS = [
  {
    title: "公式の正誤表を確認",
    body: "出版社や著者が公式サイトで正誤表を公開していることがあります。すでに掲載されている誤りは投稿不要です。投稿フォームでは、その本の正誤表が登録済みならリンクを表示します。",
  },
  {
    title: "書籍を検索",
...
const RULES = [
  ...
  "投稿の前に公式の正誤表を確認してください（掲載済みの誤りは投稿不要です）。",
] as const;

// 出版社からの回答の説明（要約。正文は利用規約の「出版社等による回答」）
const PUBLISHER_ANSWERS = [
  "回答できるのは、ステータスが「連絡済み」になった後の投稿だけです。「未対応」の間は投稿者が本文を修正できるため、回答が宙に浮かないようにしています。",
    ...
    <div className="space-y-10">
        ...
        <section>
          ...
          <p className="text-sm text-gray-500 mb-4">
            投稿は運営者と出版社・著者のやり取りに応じて、次のステータスへ移っていきます。
            「未対応」の間は投稿者が内容を修正でき、投稿を取り下げる（削除する）こともできます。「連絡済み」になった後は、内容を変えずに追記と画像の追加ができます。
          </p>
